...
namespace TaskList.Controllers
{
    ...
    public class TasksController: Controller
    {
        ...
        public TasksController(TasksContext dbContext, ILogger<TasksController> logger, IMapper automapper, IStringLocalizer<TasksController> localizer)
        {
            DbContext = dbContext;
            Logger = logger;
            Mapper = automapper;
        ...
        public async Task<IActionResult> Create([FromQuery] string title, [FromQuery] string description, [FromQuery] IEnumerable<Guid> categories, CancellationToken ct)
        {
            if (title == null)
            {
                // Will retrieve a culture-specific string from resources, if one is available.
                // Otherwise, will return the key ("TASK_TITLE_REQUIRED").
                return BadRequest(Localizer["TASK_TITLE_REQUIRED"].Value);
            }
